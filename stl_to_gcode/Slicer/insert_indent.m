function [gcode] = insert_defect(gcode_file_name, x_start_defect, y_start_defect, z_start_defect, x_size, y_size, z_size, cube_length, cube_width, cube_height)
    ...
    while i <= gcode_array_length
        if startWith(gcode_array{i}, "G01 Z")
            ...
            z_coord = zcoord{2};
            if z_coord >= y_start_defect && z_coord <= (z_start_defect + z_size)
            ...
            if is_at_defect_layer_flag == 1 && startWith(gcode_array{i}, "G01 X")
                if previous_coords == ""
                    ...
                    if previous_x_coord >= x_start_defect && previous_x_coord <= x_size && previous_y_coord >= y_start_defect && previous_y_coord  <= y_size && x_coord >= x_start_defect && x_coord <= x_size && y_coord >= y_start_defect && y_coord  <= y_size
                        previous_coords = gcode_array{i};
                    ...
                    else if previous_x_coord < x_start_defect && previous_y_coord >= y_start_defect && previous_y_coord <= y_size && x_coord >= x_start_defect && x_coord <= x_size && y_coord >= y_start_defect && y_coord <= y_size
                        previous_coords = gcode_array{i};
                    ...
                    else if previous_x_coord > x_size && previous_y_coord >= y_start_defect && previous_y_coord <= y_size && x_coord >= x_start_defect && x_coord <= x_size && y_coord >= y_start_defect && y_coord <= y_size
                        previous_coords = gcode_array{i};
                        formatSpec = "G01 X%.4f Y%.4f\n";
                        border_coord = sprintf(formatSpec, x_size, previous_y_coord);
                        gcode_array = {gcode_array{1:i-2} "M201" gcode_array{i-1:end}};
                        gcode_array = {gcode_array{1:i} border_coord gcode_array{i+1:end}};
                        gcode_array = {gcode_array{1:i+1} "M202" gcode_array{i+2:end}};
                        previous_x_coord = x_coord;
                    ...
                    else if previous_x_coord < x_start_defect && previous_y_coord >= y_start_defect && previous_y_coord <= y_size && x_coord > x_size && y_coord >= y_start_defect && y_coord <= y_size
                        previous_coords = gcode_array{i};
                        formatSpec = "G01 X%.4f Y%.4f\n";
                        left_border_coord = sprintf(formatSpec, x_start_defect, previous_y_coord);
                        right_border_coord = sprintf(formatSpec, x_size, previous_y_coord);
                        gcode_array = {gcode_array{1:i-2} "M201" gcode_array{i-1:end}};
                    ...
                    else if previous_x_coord > x_size && previous_y_coord >= y_start_defect && previous_y_coord <= y_size && x_coord < x_size && y_coord >= y_start_defect && y_coord <= y_size
                        previous_coords = gcode_array{i};
                        formatSpec = "G01 X%.4f Y%.4f\n";
                        right_border_coord = sprintf(formatSpec, x_size, previous_y_coord);
                        left_border_coord = sprintf(formatSpec, x_start_defect, previous_y_coord);
                    ...
                    else if previous_x_coord >= x_start_defect && previous_x_coord <= x_size && previous_y_coord > y_size && x_coord >= x_start_defect && x_coord <= x_size && y_coord >= y_start_defect && y_coord <= y_size
                        previous_coords = gcode_array{i};
                        formatSpec = "G01 X%.4f Y%.4f\n";
                    ...
                    else if previous_x_coord >= x_start_defect && previous_x_coord <= x_size && previous_y_coord < y_start_defect && x_coord >= x_start_defect && x_coord <= x_size && y_coord >= y_start_defect && y_coord <= y_size
                        previous_coords = gcode_array{i};
                        formatSpec = "G01 X%.4f Y%.4f\n";
