        ...
        if startWith(gcode_array{i}, "G01 Z")
            ...
            if is_at_indent_layer_flag == 1 && startWith(gcode_array{i}, "G01 X")
                if previous_coords == ""
                    ...
                    %The current line starts outside the indent and goes rightward and stops somewhere inside the indent
                    else if previous_x_coord < x_start_indent && previous_y_coord >= y_start_indent && previous_y_coord <= y_size && x_coord >= x_start_indent && x_coord <= x_size && y_coord >= y_start_indent && y_coord <= y_size
                        previous_coords = gcode_array{i};
                        formatSpec = "G01 X%.4f Y%.4f\n";
                        border_coord = sprintf(formatSpec, x_start_indent, previous_y_coord);
                    ...
                    %The current line starts outside the indent and goes downwards and stops somewhere inside the indent
                    else if previous_x_coord >= x_start_indent && previous_x_coord <= x_size && previous_y_coord > y_size && x_coord >= x_start_indent && x_coord <= x_size && y_coord >= y_start_indent && y_coord <= y_size
                        previous_coords = gcode_array{i};
                        formatSpec = "G01 X%.4f Y%.4f\n";
